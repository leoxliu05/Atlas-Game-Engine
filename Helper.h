...
public:
    ...
    static void Log(const std::string& message) // api
    {
        std::cout << message << std::endl;
    }

    static void Quit() // api
    {
        exit(0);
    }

    static void Sleep(int milliseconds) // api
    {
        std::this_thread::sleep_for(std::chrono::milliseconds(milliseconds));
    }

    static int GetFrame() // api
    {
        return GetFrameNumber();
    }

    static void OpenURL(const std::string& url) // api
    {
#ifdef _WIN32
